        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              <Link to="/pets" className="text-sm font-medium hover:text-brand transition-colors">All Pets</Link>
              {user && (
                <>
                  <Link to="/dashboard/my-requests" className="text-sm font-medium hover:text-brand transition-colors">My Requests</Link>
                  <Link to="/dashboard/add-pet" className="text-sm font-medium hover:text-brand transition-colors">Add Pet</Link>
                </>
              )}
            </div>
            ...
            <div className="hidden md:flex items-center gap-4">
              ...
              {user ? (
                <div className="relative">
                  ...
                  <AnimatePresence>
                    {isUserMenuOpen && (
                      ...
                      >
                        <Link to="/dashboard/my-listings" onClick={() => setIsUserMenuOpen(false)} className="block px-4 py-2 text-sm hover:bg-white/5">Dashboard</Link>
                        <button 
                ...
                <Link to="/login" className="px-6 py-2 bg-brand hover:bg-brand-hover text-white rounded-full text-sm font-semibold transition-all">
                  Login
                </Link>
              )}
            </div>
          ...
          {isMenuOpen && (
            ...
            >
              <div className="px-4 py-6 flex flex-col gap-4">
                ...
                <Link to="/pets" onClick={() => setIsMenuOpen(false)} className="text-lg font-medium">All Pets</Link>
                {user && (
                  <>
                    <Link to="/dashboard/my-requests" onClick={() => setIsMenuOpen(false)} className="text-lg font-medium">My Requests</Link>
                    <Link to="/dashboard/add-pet" onClick={() => setIsMenuOpen(false)} className="text-lg font-medium">Add Pet</Link>
                    <Link to="/dashboard/my-listings" onClick={() => setIsMenuOpen(false)} className="text-lg font-medium">My Listings</Link>
                    <button onClick={logout} className="text-lg font-medium text-red-400 text-left">Logout</button>
                  </>
                )}
                {!user && (
                  <Link to="/login" onClick={() => setIsMenuOpen(false)} className="text-lg font-medium text-brand">Login</Link>
                )}
              </div>
            </motion.div>
          )}
